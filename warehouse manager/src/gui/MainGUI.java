...
public class MainGUI extends JFrame {
    ...
    public MainGUI() {
        ...
        String[] cols3 = {"Item Code", "Driver Code", "Driver Name", "Plate Number", "Quantity"};
        ...
        cancelCourierBtn.addActionListener(e -> {
            ...
            if (r >= 0) {
                String itemCode = (String) deliveryTable.getValueAt(r, 0);
                String truckCode = (String) deliveryTable.getValueAt(r, 1);
                String driverName = (String) deliveryTable.getValueAt(r, 2);
                String plate = (String) deliveryTable.getValueAt(r, 3);

                Object qtyObj = deliveryTable.getValueAt(r, 4);
                int qty;
                if (qtyObj instanceof Integer) qty = (Integer) qtyObj;
                else qty = Integer.parseInt(qtyObj.toString());

                // Restore the truck with original driver name
                Truck cancelledTruck = new Truck(truckCode, driverName, plate);
                courierManager.acceptTruck(cancelledTruck);

                // Restore the item to storage
                List<Item> items = storageManager.getAllItems();
                Item existing = null;
                for (Item i : items) {
                    if (i.getCode().equals(itemCode)) { existing = i; break; }
                }

                if (existing != null) {
                    existing.setQuantity(existing.getQuantity() + qty);
                } else {
                    storageManager.addItem(new Item(itemCode, "Restored Item", qty));
                }

                // Remove cancelled delivery and refresh
                deliveryModel.removeRow(r);
                refreshStorageTable();
                refreshCourierTable();
